...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
			int[] testPoints = {1, 1,  board.length - 2, board[0].length - 2, board.length / 2, board[0].length / 2}; 
	...
	public static int[][] evolve(int[][] board) {
		...
		for (int k = 1; k < board.length - 1; k++) {
			for (int l = 1; l < board[0].length - 1; l++) {
				int aliveNeighbours = count(board, k, l);
				if (cellValue(board, k, l) == 1) {
					if (aliveNeighbours < 2) {
						nextGeneration[k][l] = 0; 
					} else if (aliveNeighbours == 2 || aliveNeighbours == 3) {
						nextGeneration[k][l] = 1; 
	...
	public static int count(int[][] board, int i, int j) {
		int count = 0;
		count += cellValue(board, i-1, j-1);
		count += cellValue(board, i-1,j);
		count += cellValue(board, i-1,j+1);

		count += cellValue(board,i,j-1);
		count += cellValue(board,i,j+1);
